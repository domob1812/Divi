...
protected:
    ...
    : coins(&coinsDummy), rewards(CENT, 0, 0, 0, 0, 0)
  {
    ...
    coins.ModifyCoins(dummyTxVault.GetHash2())->FromTx(dummyTxVault, 0);
    coins.ModifyCoins(dummyTxNonVault.GetHash2())->FromTx(dummyTxNonVault, 0);

    for (unsigned i = 0; i < dummyTxVault.vout.size(); ++i) {
      vaultCoins.emplace_back(dummyTxVault.GetHash2(), i);
      nonVaultCoins.emplace_back(dummyTxNonVault.GetHash2(), i);
    }
  }
